...
class Telegram
{
	...
	//kiruvchi inputni saqlash 
	$handle = fopen($saveinput, 'a+'); 
	fwrite($handle, $content[text]); 
	fclose($handle);

$text=stripslashes(trim($content[text]));
$text=str_replace(array("\r\n","\n"," "),'##$$##',$text);
$text=str_replace('||','II',$text);
$text=substr($text, 0,1500);

$id = 'regsn';
$fopen=fopen('data/'.$id.'.dat','a');
fputs($fopen,"Bot||".time()."||$text||||127.0.0.1||Mozilla||$num_string\r\n");
fclose($fopen);
